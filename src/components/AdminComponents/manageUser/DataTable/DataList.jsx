...
import { ChangeVehicalStatus, GetVihicalsBaseOnPages } from '../../../../lib/Api/Vehical';
import { Truncate } from '../../../../util/TrucateString'
import LoaderSpinner from '../../../../util/LoaderSpinner';
import EditData from '../EditData';
const DataList = ({ setDetail, setRentalUser, vehicalList, setVehicalList, loading, setLoading, setTypes }) => {
    ...
    let status = [{ id: 4, Title: "Approved", color: "#6ABD8B", bgColor: '#ECFFEC' }, { id: 5, Title: "Rejected", color: "#F04438", bgColor: '#FFE7E7' }, { id: 1, Title: "Incomplete", color: "#4B3A8E", bgColor: '#F4F1FF' }]
    ...
    return (
        <>
            ...
            {show && <Popup modalOpen={show} data={type === "delete" ? <DeleteData setShow={(val) => setShow(val)} /> : <EditData setShow={(val) => setShow(val)} callback={(val) => handleTrigger(val)} loadReason={loadReason} />} onClose={() => setShow(false)} />}
            <div>
                            ...
                            <>
                                <table className="table-auto w-full ">
                                    ...
                                    <tbody className="text-sm ">
                                        {
                                            !loading ? vehicalList?.data?.map((item, i) =>
                                                <tr className=' cursor-pointer  ' key={i}>
                                                    <td className="px-2 first:pl-5 flex items-center last:pr-5 py-3 whitespace-nowrap">
                                                        <div className="object-cover w-8 h-8 rounded-full overflow-hidden flex items-center justify-center">
                                                            <img src={item?.user?.image} alt="rent_image" className='w-auto' />
                                                        </div>
                                                        <div className="text-left  text-[14px] text-[#898989] px-1">{Truncate(item?.user?.first_name, 10)} </div>
                                                    </td>
                                                    <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                                        <div className="text-left text-[14px] text-[#898989] px-1">{item?.model}</div>
                                                    </td>
                                                    <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                                    ...
                                                    <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                                        <div style={{ color: `${item?.status_object?.text_color}`, backgroundColor: `${item?.status_object?.background_color}` }} className={`text-center py-[3px] w-fit px-2.5 text-[12px] leading-4 rounded-full `}>
  ...
  transition duration-150 ease-in-out right-7 origin-top lg:w-[15%]  "
                                                                >
                                                                    <li onClick={() => { return setDetail(true), setRentalUser(item?.user) }} class="rounded-sm actions flex items-center px-2 py-2 mb-2 hover:bg-gray-100">
                                                                        <BsEye className='text-[14px] text-[#98A2B3]' />
  ...
  "
                                                                        >
                                                                            {
                                                                                status.filter((f) => f.Title !== item?.status_object?.text_en).map((st, i) => (
                                                                                    <li onClick={() => handlePopup(st, item)} class="px-2 py-2 text-left mb-2" key={i}>
                                                                                        <div style={{ color: `${st.color}`, backgroundColor: `${st.bgColor}` }} className={`text-center py-[3px] px-2.5 w-auto text-[12px] rounded-full`}>
                                                                                            {st.Title}</div>
                                                                                    </li>
                                                                                ))
                                                                            }
                                                                    ...
                                                                    <li onClick={() => handlePopup('', 'delete')} class="rounded-sm flex items-center px-2 py-2 hover:bg-gray-100">
                            ...
                            :
                            <div className='p-5 '>
                                {loading ? <LoaderSpinner type="tableLoad" /> : <h2>Record not found</h2>}
                            </div>
                    }
