...
const ManageRole = () => {
  ...
  useEffect(() => {
    let fetchRols = async () => {
      ...
      setRole(
        rols.roles.map((obj) => {
          return { label: obj.role, value: obj.role };
        })
      );
    };
    fetchRols();
      ...
      <div className="bg-white shadow-[0_4px_20px_0px_rgba(0,0,0,0.08)] mt-5 p-5 rounded-md">
        <div className="flex justify-between">
          <div>
            ...
            <p className="pt-4 text-[#898989] text-[14px]">
              Here you can add role to Members
            </p>
          </div>
        </div>
          ...
          <div className="grid lg:grid-cols-3 md:grid-cols-3 pt-5 gap-[34px] ">
            <div className="flex flex-col">
              ...
              <input
                ...
                {...register("name")}
                placeholder="Name"
                className={`border border-[#CDD2E1] h-[42px] placeholder:text-[14px] text-[14px] rounded-md text-[#ACACAC] focus:outline-none px-2.5 ${
